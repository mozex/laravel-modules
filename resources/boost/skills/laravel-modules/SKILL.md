...
description: Detailed implementation patterns for mozex/laravel-modules (2.x branch, Livewire v3 and Filament v3/v4). Activate when working with module routes (custom route groups, custom registrars, inline Route::localized, route file organization by third-party concern like fortify.php), Livewire v3 components inside modules, Filament v3/v4 resources/pages/widgets/clusters across panels, module-level model factories or policies, config merging between modules and the app, overriding module views from the host application, event listener auto-discovery, Blade components inside modules, seeder wiring via Modules::seeders(), per-module Console Kernel schedules, module service providers for morph maps and third-party model policies, when setting up PHPStan/Pest/PHPUnit for a modular Laravel project, or when organizing Inertia frontend assets per module with Vue or React (Vite alias, Inertia page resolver, cross-module imports, typed props from PHP via Spatie TypeScript Transformer). Also covers the full Mozex\Modules\Facades\Modules API.
---
...
This is the skill for the **2.x branch** of the package: Livewire v3 and Filament v3/v4. If the project uses Livewire v4 or Filament v5, it should be on the 3.x branch, which has a separate skill file with different Livewire and Filament content.
...
- Creating class-based Livewire v3 components inside a module
- Registering Filament v3/v4 resources, pages, widgets, or clusters with specific panels
...
- Registering scheduled tasks in modules (Routes/console.php recommended, or per-module Console Kernel as alternative)
...
This version of the package targets Livewire v3. It discovers class-based components from each module's `Livewire/` directory and registers them with namespaced aliases. The feature is conditional on `class_exists(Livewire::class)`, so nothing happens if Livewire isn't installed. Livewire v4 features like single-file components (SFC) and multi-file components (MFC) are not supported on this branch; for v4 support, use the 3.x version of the package.
...
Used as `<livewire:blog::post-editor />`. Nested directories become dot-separated aliases: `Livewire/Comments/CommentList.php` → `<livewire:blog::comments.comment-list />`.
...
No `view_path` option on this branch. The package registers each discovered component via `Livewire::component($name, $class)`, not via `Livewire::addNamespace()`, because v3 doesn't have the SFC/MFC namespace discovery mechanism.

## Filament

This version of the package supports Filament v3 and v4. It discovers Resources, Pages, Widgets, and Clusters across modules and registers them with the matching Filament panel via `$panel->discoverResources()`, `$panel->discoverPages()`, and so on (APIs that exist in both v3 and v4). For Filament v5, use the 3.x branch of the package.

The feature is conditional on `class_exists(Filament::class)`.
...
The package's discovery only cares about the outer `Filament/{Panel}/{Resources|Pages|Widgets|Clusters}/` pattern. The internal structure of each resource is up to you. The v4 convention uses `Schemas/` and `Tables/` subdirectories alongside `Pages/` inside each resource directory. The v3 convention uses a flat resource file with `form()` and `table()` methods directly on the class, with only `Pages/` and `RelationManagers/` as subdirectories. Both work because the package's discovery doesn't look inside resource directories.
...
The recommended way to register scheduled tasks in modules is through `Routes/console.php` using the `Schedule` facade. This is handled by the Routes feature and documented in its section above.

The 2.x branch also ships a per-module Console Kernel feature as an alternative. Create a `Console/Kernel.php` class inside a module, extend `Mozex\Modules\Contracts\ConsoleKernel`, and implement a `schedule()` method. The package discovers the kernel and calls its `schedule()` method at boot.
...
Any other class in the `Console/` directory is ignored (including Artisan commands under `Console/Commands/`, which are handled by the Commands feature).
...
Setting `'schedules.active' => false` disables Console Kernel discovery; `Routes/console.php` schedules still work because they're handled by the Routes feature.
