...
This version of the package supports Filament v3 and v4. The code examples and directory layout on this page use the Filament v4 convention (with `Schemas/`, `Tables/`, and `Filament\Schemas\Schema`). If you're on Filament v3, replace `Schema $schema` with `Form $form` from `Filament\Forms\Form` and define your form and table methods directly on the resource class instead of delegating to separate schema/table files. If you need Filament v5 support, use the [3.x version](https://github.com/mozex/laravel-modules) of this package.
